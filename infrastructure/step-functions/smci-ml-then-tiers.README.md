# Predixa-SMCI-ML-Then-Tiers

Per-ticker equity pipeline (SMCI pilot). Tiers write ratings + detail description
into `db/tickers/SMCI/features.db`.
...
| State | Function |
|-------|----------|
| Features | `predixa-features-SMCI` |
| ML1 | `predixa-ml1-SMCI` |
| ML2 | `predixa-ml2-SMCI` |
| ML3 | `predixa-ml3-SMCI` |
| Tiers | `predixa-tiers-SMCI` |

Handlers: `tradespark/ticker-handlers/SMCI/ml/model_3mix/{features,model1,model2,model3,tiers}/`.
...
aws stepfunctions create-state-machine \
  --name Predixa-SMCI-ML-Then-Tiers \
  --definition file://infrastructure/step-functions/smci-ml-then-tiers.asl.json \
