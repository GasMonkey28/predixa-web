...
| --- | --- | --- |
| Vercel Next.js | Vercel alerts, Web analytics, Sentry (`SENTRY_DSN`) | Track slow endpoint logs; raise Sentry alerts on uncaught errors. |
| Stripe | Dashboard → Developers → Logs | Verify webhook deliveries, subscription events, portal sessions. |
| AWS Lambda (entitlements/webhooks) | CloudWatch Alarms | Set alarms on errors/timeouts; stream logs to SNS/Slack. |
...
See [`docs/monitoring.md`](./monitoring.md) for detailed CloudWatch, Vercel, and Sentry alert configurations.
