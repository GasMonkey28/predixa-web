...
// Get last 40 trading days (excluding weekends)
function getLast40TradingDays(): string[] {
  ...
  while (tradingDays.length < 40 && daysBack < 80) {
...
export async function GET() {
  try {
    ...
    let ohlcData: any[] = []
    try {
      const yahooUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/SPY?interval=1d&range=40d'
      ...
      if (chartData) {
        ...
        // Sort by date (oldest first) and take last 40
        ohlcData.sort((a, b) => a.timestamp - b.timestamp)
        ohlcData = ohlcData.slice(-40)
      }
    ...
    logger.error({ error, message: error?.message }, 'Unhandled error in history-40 API')
    return NextResponse.json(
