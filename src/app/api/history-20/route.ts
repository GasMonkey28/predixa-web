...
// Get last 20 trading days (excluding weekends)
function getLast20TradingDays(): string[] {
  ...
  while (tradingDays.length < 20 && daysBack < 40) {
...
export async function GET() {
  try {
    ...
    let ohlcData: any[] = []
    try {
      const yahooUrl = 'https://query1.finance.yahoo.com/v8/finance/chart/SPY?interval=1d&range=20d'
      ...
      if (chartData) {
        ...
        // Sort by date (oldest first) and take last 20
        ohlcData.sort((a, b) => a.timestamp - b.timestamp)
        ohlcData = ohlcData.slice(-20)
      }
    ...
    logger.error({ error, message: error?.message }, 'Unhandled error in history-20 API')
    return NextResponse.json(
