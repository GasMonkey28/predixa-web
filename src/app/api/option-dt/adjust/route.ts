...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const position = positions.find(
      (p) => p.Symbol === symbol && isOptionSymbol(p.Symbol, p.AssetType)
    )
    if (!position) {
      return NextResponse.json(
        { error: `No open option position for ${symbol}` },
        { status: 404 }
      )
    }
    ...
    if (action === 'buy_more') {
      const addQty = Math.max(1, Math.min(Math.floor(body.quantity ?? 1), 20))
      order = {
        AccountID: accountId,
        Symbol: symbol,
      ...
      order = {
        ...
        OrderType: 'Market',
        TradeAction: isLong ? 'SELLTOCLOSE' : 'BUYTOCLOSE',
        TimeInForce: { Duration: 'DAY' },
        Route: 'Intelligent',
      }
    } else {
    ...
    logger.error({ error, userId: auth.userId }, 'Option DT adjust error')
    return NextResponse.json(
