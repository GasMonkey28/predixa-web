...
function TickersInsightPageContent() {
  const searchParams = useSearchParams()
  const fromQuery = searchParams.get('ticker')?.toUpperCase()
  const initial: EquityTicker =
    fromQuery && isEquityTicker(fromQuery) ? fromQuery : DEFAULT_EQUITY_TICKER
  const [ticker, setTicker] = useState<EquityTicker>(initial)
      ...
      <div className="relative mx-auto max-w-7xl p-6">
        ...
        >
          ...
          <div className="space-y-4">
            <section className="rounded-xl border border-zinc-700/60 bg-zinc-900/50 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-medium text-zinc-200">3mix letter tiers</h3>
              </div>
            ...
            <section className="rounded-xl border border-zinc-700/60 bg-zinc-900/50 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-sm font-medium text-zinc-200">Model 2 (y2y3)</h3>
              </div>
          ...
          <p className="text-xs text-zinc-500 border-t border-zinc-800 pt-4 mt-5 leading-relaxed">
            For informational purposes only. Not investment advice. Equity pipelines publish under
            ticker-scoped S3 keys; SPY Summary remains unchanged.
          </p>
        </motion.div>
      </div>
    </div>
...
export default function TickersInsightPage() {
  return (
    <ProtectedRoute requireSubscription>
      <Suspense
        fallback={
          <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-gray-900 flex items-center justify-center text-zinc-300">
            Loading insight…
          </div>
        }
      >
        <TickersInsightPageContent />
      </Suspense>
    </ProtectedRoute>
  )
}
